...
impl ArbEngine {
    ...
    fn calculate_arbitrage(
        ...
        if net_profit_bps <= 0.0 {
            info!(
                "No arbitrage opportunity: net profit {:.2}bps (buy: ${:.4}, sell: ${:.4})",
                net_profit_bps, buy_price, sell_price
            );
            return Ok(None);
        }
    ...
    async fn alert_opportunity(&self, opportunity: ArbOpportunity) {
        let action_text = match opportunity.direction {
            ArbDirection::BuyCex => format!(
                "buy CEX @ ${:.4}, SELL DEX @ ${:.4}",
                opportunity.cex_price, opportunity.dex_price
            ),
            ArbDirection::BuyDex => format!(
                "buy DEX @ ${:.4}, SELL CEX @ ${:.4}",
                opportunity.dex_price, opportunity.cex_price
            ),
        };
